 ...
 *  - io is expensive, store it in a good ds
 ...
 *  - a synset can consist of exactly one noun and several different synsets can
 *    consist of that same single noun.
...
public class WordNet {
     ...
     * verticies in the graph. ie a meaning of a noun. each line:
     * int, noun0 noun*, gloss
     * 
     * Polymorphism:
     * - List is an interface, ArrayList is an implemention of List
     * - ArrayList is a resizable array implementsino of the List interface
     * List<Integer> nids = new ArrayList<Intger>
     * - ArrayList overrides list methods,
     * - can only call List interface methods
     * - preferred.. as this defines what methods an inheriting class
     *   api or client can use without sharing what the implementation is.
     * - allows the implementation to be swapped out at a later point 
     * ArrayList<Integer> nids = new ArrayList<Integer>
     * - can call any extra methods defined in ArrayList
     *   beyond the list interface
     *   
     * Hash[String:noun] ->[id0,..idn]
     */
    ...
    private void importHypernyms(String file, Digraph dg) {
        ...
        } // we are a DAG; ensure there is a single root.
        
        int numRoots = 0;
        // clunky outdegree; no digraph methods allow for edgelist size
        ...
        if (numRoots != 1) {
            throw new java.lang.IllegalArgumentException("Found"+ numRoots +" roots. 1 root required.");
        }

        /*
        // root will be at the end of this list; reversable?
        for (Integer i: topological.order()) {
            StdOut.println(synsetHash.get(i) + " " + i);
        }
        StdOut.println("^--- Should be Root");
        */
     ...
     * hypernyms is the list of directed edges connecting synset nouns by id:
     164,21012,56099
     * the nouns are not unique, but their ids are, which is how the edges
     * are connected.
     */
    public WordNet(String synsets, String hypernyms) {
        ...
        sap = new SAP(dg); // copy to immutable graph to free dg reference
        
        //StdOut.println("Digraph: "+dg);
    }
    ...
    public static void main(String[] args) {
        ...
        StdOut.println("a->f:" + wn.distance("a","f"));
        /*
